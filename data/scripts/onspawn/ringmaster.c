...
void main()
{
    ...
    float baseScore        = getglobalvar("pBase")*12;
    float iMHealth;
    float iHealth;
    float iAggre;
    float iScore;
    ...
    if(level == 4){vAlias = "Blaster";}else
    if(level == 3){vAlias = "Buster";}else
    if(level == 2){vAlias = "Thruster";}else
    if(level == 1){vAlias = "Ringlord";}else
    {vAlias = "Ringmaster";}
