...
{//Opponent forced do Land and change to Pain if hit in mid-air (KOF RYO DESPERATION STYLE)
    ...
    if(Ty > tBase && entityAlive(target) && targetInvincible == 0){
        if(eType != openborconstant("TYPE_OBSTACLE") && sType != openborconstant("SUBTYPE_NOTGRAB")){
            ...
            if(vAniID == openborconstant("ANI_GRABDOWN") && frame < 8){
                setidle(target);
                changeentityproperty(target,"position", Tx, Tz, 0);
                changeentityproperty(target,"velocity", 0, 0, 0);
                executeanimation(target, openborconstant("ANI_PAIN"), 1);
            ...
            if(vAniID == openborconstant("ANI_FOLLOW2") && frame < 58){
                setidle(target);
                changeentityproperty(target,"position", Tx, Tz, 0);
                changeentityproperty(target,"velocity", 0, 0, 0);
                executeanimation(target, openborconstant("ANI_PAIN"), 1);
            ...
            if(vAniID == openborconstant("ANI_FOLLOW3") && frame > 13 && frame < 16){
                setidle(target);
                changeentityproperty(target,"position", Tx, Tz, 0);
                changeentityproperty(target,"velocity", 0, 0, 0);
                executeanimation(target, openborconstant("ANI_PAIN"), 1);
            ...
            if(vAniID == openborconstant("ANI_FREESPECIAL") && frame >=4){performattack(self, openborconstant("ANI_FOLLOW2"), 0);}
            if(vAniID == openborconstant("ANI_FREESPECIAL4") && frame >=4){performattack(self, openborconstant("ANI_FOLLOW3"), 0);}
        }
    }
}
