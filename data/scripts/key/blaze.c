...
{//Cancel defined moves with Rage Attacks
    ...
    if(playerkeys(iPIndex, 0, "Movedown") && mp >= costMp && rush >= 1 && hits >= 1){
        if(playerkeys(iPIndex, 1, "special")){
            if(vAniID == openborconstant("ANI_ATTACK3") && height <= base){
                mpCost(costMp);
            ...
            if(vModel == "Blaze_Knife" && vAniID == openborconstant("ANI_ATTACK1")){
                mpCost(costMp/3);
                setglobalvar("rageCancel"+self, openborvariant("elapsed_time"));
                performattack(self, openborconstant("ANI_FREESPECIAL"), 0);
            }
...
{//Cancel defined moves with Special Attacks
    ...
    int cost2     = getentityproperty(self,"energycost", "cost", openborconstant("ANI_SPECIAL2"));
    ...
    if(playerkeys(iPIndex, 1, "attack")){
        ...
        if(vAniID == openborconstant("ANI_FREESPECIAL") && vModel == "Blaze_Knife"){
            if((mp >= 3 || gp >= gpCostAmt) && frame > 2 && frame < 65 && rush > 1 && hits > 5  && base == height){
                if(gp >= gpCostAmt) {
                    gpCost(gpCostAmt);
                } else {
                    mpCost(1);
    ...
    if(playerkeys(iPIndex, 1, "special")){
        if(playerkeys(iPIndex, 0, "moveleft") && dir == 0 || playerkeys(iPIndex, 0, "moveright") && dir == 1){
            ...
            if(rush >= 1 && hits >= 1){
                ...
                if(vAniID == openborconstant("ANI_RUNATTACK") && height == base){
                    if(vModel == "Blaze_Knife" || vModel == "Blaze_Sword"){
                        mpCost(cost2);
                        changeentityproperty(self, "velocity", 0, 0, 0);
                        setglobalvar("specialCancel"+self, openborvariant("elapsed_time"));
                        performattack(self, openborconstant("ANI_SPECIAL2"), 0);
            ...
            // if(mp >= cost && height == base){
            //     gpCost(gpCostAmt);
            //     changeentityproperty(self, "velocity", 0, 0, 0);
            //     performattack(self, openborconstant("ANI_ATTACK7"), 0);
            // }
        }
        else if(playerkeys(iPIndex, 0, "MoveDown")){
            // if(gp >= gpCostAmt && height == base){
            //     mpCost(cost);
            //     changeentityproperty(self, "velocity", 0, 0, 0);
            //     performattack(self, openborconstant("ANI_ATTACK8"), 0);
            // }
        }
        else
        {
...
{//Cancel defined moves with Offensive Jump Special (Special3)
    ...
    if(playerkeys(iPIndex, 1, "special")){
        if((mPonly == 1 || mPonly == 0) && mp >= cost){
            if(vAniID == openborconstant("ANI_RUNATTACK") && rush >= 1 && hits >= 1 && tHeight > tBase){
            ...
            if(vAniID == openborconstant("ANI_FREESPECIAL2") && rush >= 1 && hits >= 1 && height > base){
                mpCost(cost*costRate);
                tossentity(self, Vy, xDir, zDir);
                setglobalvar("specialCancel"+self, openborvariant("elapsed_time"));
                performattack(self, openborconstant("ANI_FOLLOW8"), 0);
