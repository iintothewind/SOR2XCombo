...
{//Cancel defined moves with Rage Attacks
    ...
    int costMp2     = getentityproperty(self,"energycost", "cost", openborconstant("ANI_FREESPECIAL4"));
    float time     = openborvariant("elapsed_time");

    if(playerkeys(iPIndex, 0, "Movedown") && mp >= costMp && rush >= 1 && hits >= 1){
        if(playerkeys(iPIndex, 1, "special")){
            if(vAniID == openborconstant("ANI_ATTACK4") || vAniID == openborconstant("ANI_FOLLOW7")){
                mpCost(costMp);
            ...
            if((vAniID == openborconstant("ANI_SPECIAL2")
             ...
             || vAniID == openborconstant("ANI_FREESPECIAL21")) && height == base){
                mpCost(costMp);
    ...
    if(playerkeys(iPIndex, 0, "MoveUp") && mp >= costMp2 && rush >= 1 && hits >= 1 && height == base){
        if(playerkeys(iPIndex, 1, "special")){
            if(vAniID == openborconstant("ANI_ATTACK4") || vAniID == openborconstant("ANI_FOLLOW7")){
                mpCost(costMp2);
                setglobalvar("rageCancel"+self, openborvariant("elapsed_time"));
...
{//Cancel defined moves with Special Attacks
    ...
    if(playerkeys(iPIndex, 1, "attack")){
        ...
        if(vAniID == openborconstant("ANI_SPECIAL2")){
            if(gp >= gpCostAmt && frame >= 12 && frame <= 17 && rush > 1 && hits > 1 && base == height){
                gpCost(gpCostAmt);
                mpCost(-1);
    ...
    if(playerkeys(iPIndex, 1, "special")){
        ...
        else if(playerkeys(iPIndex, 0, "moveup"))
        {
            if(rush >= 1 && hits >= 1){
                ...
                float gp         = getentityproperty(self,"guardpoints");
                float gpCostAmt    = maxGp/3;
                if(gp >= gpCostAmt && (vAniID == openborconstant("ANI_ATTACK4")
                || vAniID == openborconstant("ANI_FREESPECIAL20")
                || vAniID == openborconstant("ANI_FREESPECIAL21")
...
void kiko()
{
    ...
    if(playerkeys(iPIndex, 1, "attack4") && playerkeys(iPIndex, 0, "moveup")){
        if(    vAniID == openborconstant("ANI_IDLE")    ||
            ...
            if(getentityproperty(self, "animvalid", openborconstant("ANI_FREESPECIAL5")) == 1 && gp >= gpCostAmt){
                gpCost(gpCostAmt);
                setidle(self, openborconstant("ANI_IDLE"));
                changeentityproperty(self, "velocity", 0, 0, 0);
                performattack(self, openborconstant("ANI_FREESPECIAL5"), 1);
            } else {
