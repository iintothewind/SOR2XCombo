...
{//This script activates a Debug Mode to make adjustments on GRAB positions, frame by frame
 //To use correctly, copy this line inside your ondraw script file in the first line: #import "data/scripts/_debugs/DEBUG_GRAB.c"
 //If your entity doesn't have an ondraw script event, need to create one. You can call the script using debugGrab(); function
 //You can call an ondraw event in any character using this code at the header: ondrawscript data/scripts/_debugs/DEBUG_GRAB.c
 ...
 //Jump					- change grabbed entity facing direction
	...
	if(openborvariant("in_level")){ //IN ANY LEVEL?
		if(openborvariant("pause") == 0 && openborvariant("in_options") != 1){ //NOT PAUSED AND OR IN OPTIONS?
	...
	if(getglobalvar("debugMode") == "on"){ //IS IN DEBUG MODE??
		...
		if(playerkeys(0, 0, "screenshot")){ //USED TO UNFREEZE THE GAME SLOWLY
			if(openborvariant("nopause") != 0){ //USED TO RUN ONCE
				changeopenborvariant("nopause", 0); //UNLOCK PAUSE COMMAND
				changeopenborvariant("textbox", 0); //CLEAR TEXTBOX TO UNFREEZE THE GAME
			}
		}else{
			if(openborvariant("nopause") != 1){ //USED TO RUN ONCE
		...
		if(getlocalvar("background") == NULL()){
			void background = loadsprite("data/sprites/back1.png"); //LOAD BACKGROUND FOR FURTHER USE
